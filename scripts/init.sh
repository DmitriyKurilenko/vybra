...
#   3. Получение TLS-сертификата Let's Encrypt (standalone, ~30 сек)
#   4. Настройка nginx в HTTPS-режиме
#   5. Первый деплой приложения (docker compose up + migrate + collectstatic)
#
# Использование:
...
if [[ $TOTAL_RAM_MB -lt 2048 ]] && ! swapon --show | grep -q /swapfile; then
  SWAP_SIZE="1G"
  log "RAM ${TOTAL_RAM_MB}MB < 2GB — создаю swapfile ${SWAP_SIZE} (Chromium требует запас)..."
  fallocate -l "$SWAP_SIZE" /swapfile
  chmod 600 /swapfile
  mkswap /swapfile
  swapon /swapfile
  echo '/swapfile none swap sw 0 0' >> /etc/fstab
  # Снижаем swappiness — swap только как страховка
